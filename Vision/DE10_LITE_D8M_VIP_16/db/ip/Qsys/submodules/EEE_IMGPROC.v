...
wire magenta_detect_h, magenta_detect_s, magenta_detect_v;
wire teal_detect_h, teal_detect_s, teal_detect_v;
wire lime_detect_h, lime_detect_s, lime_detect_v;
wire navy_detect_h, navy_detect_s, navy_detect_v;
wire building_detect_h, building_detect_s, building_detect_v;

wire         sop, eop, in_valid, out_ready;
////////////////////////////////////////////////////////////////////////

// Hue, Saturation and Value thresholds for aliens and building
assign red_detect_h = ((9'd20< hsv_h && hsv_h < 9'd30 )|| ((9'd350 < hsv_h) & (hsv_h < 360))) ? 1'b1 : 1'b0;
assign red_detect_s = (9'd80 < hsv_s && hsv_s < 9'd255) ? 1'b1 : 1'b0;
assign red_detect_v = (9'd100 < hsv_v && hsv_v < 9'd255) ? 1'b1 : 1'b0;

//assign magenta_detect_h = (9'd300 < hsv_h  &&  hsv_h < 9'd345) ? 1'b1 : 1'b0;
assign magenta_detect_h = (hsv_h < 9'd20 || ((9'd300 < hsv_h) & (hsv_h < 335))) ? 1'b1 : 1'b0;
//assign magenta_detect_h = (hsv_h < 9'd20 ) ? 1'b1 : 1'b0;
assign magenta_detect_s = (9'd80 < hsv_s && hsv_s < 9'd255) ? 1'b1 : 1'b0;
assign magenta_detect_v = (9'd80 < hsv_v && hsv_v < 9'd255) ? 1'b1 : 1'b0;

//For red and magenta, change the saturation and value only
////////////////////////////////////////////////////////////////////////
...
assign teal_detect_h = (9'd110 < hsv_h  &&  hsv_h < 9'd140) ? 1'b1 : 1'b0;
assign teal_detect_s = (9'd30 < hsv_s) ? 1'b1 : 1'b0;
assign teal_detect_v = (9'd30 < hsv_v && hsv_v < 9'd130) ? 1'b1 : 1'b0;

assign lime_detect_h = (9'd80 < hsv_h  &&  hsv_h < 9'd110) ? 1'b1 : 1'b0;
assign lime_detect_s = (9'd100 < hsv_s) ? 1'b1 : 1'b0;
assign lime_detect_v = (9'd50 < hsv_v) ? 1'b1 : 1'b0;

assign navy_detect_h = (9'd180 < hsv_h  &&  hsv_h < 9'd240) ? 1'b1 : 1'b0;
assign navy_detect_s = (9'd30 < hsv_s) ? 1'b1 : 1'b0;
assign navy_detect_v = (9'd30 < hsv_v) ? 1'b1 : 1'b0;

assign building_detect_h = (9'd50 < hsv_h & hsv_h < 9'd85) ? 1'b1 : 1'b0;
assign building_detect_s = (hsv_s < 9'd100) ? 1'b1 : 1'b0;
assign building_detect_v = (9'd170 < hsv_v) ? 1'b1 : 1'b0;
...
reg [10:0] m_left, m_right, m_top, m_bottom;
reg [10:0] t_left, t_right, t_top, t_bottom;
reg [10:0] l_left, l_right, l_top, l_bottom;
reg [10:0] n_left, n_right, n_top, n_bottom;
...
always@(posedge clk) begin
	...
	if (msg_state != 4'b0000) 
		begin 
			...
			else msg_state <= msg_state + 4'b0000;
		end

end

//Data send to control subsystem (Successibility of detection)
//MSB will be 1 if detected
//Colour: red 00000, yellow 00001, magenta 00010, teal 00011, lime 00100, navy 00101, building 00110
reg[4:0] red_detect_ctrl, yellow_detect_ctrl, magenta_detect_ctrl, teal_detect_ctrl;
reg[4:0] lime_detect_ctrl, navy_detect_ctrl, building_detect_ctrl;

always@(posedge clk) begin

	if(red_detect_hsv & in_valid) begin
		red_detect_ctrl <= 5'b10000;
		//detected <= 16'b1;
	end else begin
		red_detect_ctrl <= 5'b00000;
	end
	
	
	if (yellow_detect_hsv & in_valid) begin 
		yellow_detect_ctrl <= 5'b10001;
		//detected <= 16'b1;
 	end else begin
		yellow_detect_ctrl <= 5'b00001;
	end


	if (magenta_detect_hsv & in_valid) begin 
		magenta_detect_ctrl <= 5'b10010;
		//detected <= 16'b1;
	end else begin
		magenta_detect_ctrl <= 5'b00010;
	end
	
	if (teal_detect_hsv & in_valid) begin 
		teal_detect_ctrl <= 5'b10011;
		//detected <= 16'b1;
	end else begin
		teal_detect_ctrl <= 5'b00011;
	end
	
	
	if (lime_detect_hsv & in_valid) begin 
		lime_detect_ctrl <= 5'b10100;
		//detected <= 16'b1;
	end else begin
		lime_detect_ctrl <= 5'b00100;
	end
		
	if (navy_detect_hsv & in_valid) begin 
		navy_detect_ctrl <= 5'b10101;
		//detected <= 16'b1;
	end else begin
		navy_detect_ctrl <= 5'b00101;
	end
		
	if (building_detect_hsv & in_valid) begin 
		building_detect_ctrl <= 5'b10110;
		//detected <= 16'b1;
	end else begin
		building_detect_ctrl <= 5'b00110;
	end
	
end

//Data send to control subsystem (center x coordinate of bounding box)
reg[10:0] r_x_cen, y_x_cen, m_x_cen, t_x_cen, l_x_cen, n_x_cen, b_x_cen;

always@(posedge clk) begin
	
	 r_x_cen <= r_x_max - r_x_min;
	 y_x_cen <= y_x_max - y_x_min;
	 m_x_cen <= m_x_max - m_x_min;
	 t_x_cen <= t_x_max - t_x_min;
	 l_x_cen <= l_x_max - l_x_min;
	 n_x_cen <= n_x_max - n_x_min;
	 t_x_cen <= t_x_max - t_x_min;
...
always@(*) begin	//Write words to FIFO as state machine advances
	case(msg_state)
		...
		4'b0010: begin
			msg_buf_in = {red_detect_ctrl, r_x_cen, red_area};
			msg_buf_wr = 1'b1;
		end
		
		4'b0011: begin
			msg_buf_in = {yellow_detect_ctrl, y_x_cen, yellow_area};
			msg_buf_wr = 1'b1;
		end
		
		4'b0100: begin
			msg_buf_in = {magenta_detect_ctrl, m_x_cen, magenta_area};
			msg_buf_wr = 1'b1;
		end
		
		4'b0101: begin
			msg_buf_in = {teal_detect_ctrl, t_x_cen, teal_area};
			msg_buf_wr = 1'b1;
		end
		
		4'b0110: begin
			msg_buf_in = {lime_detect_ctrl, l_x_cen, lime_area};
			msg_buf_wr = 1'b1;
		end
		
		4'b0111: begin
			msg_buf_in = {navy_detect_ctrl, n_x_cen, navy_area};
			msg_buf_wr = 1'b1;
		end
		
		4'b1000: begin
			msg_buf_in = {building_detect_ctrl, b_x_cen, building_area};
			msg_buf_wr = 1'b1;
		end
		...
		/*4'b0010:begin
			msg_buf_in = {5'b0, r_x_min, 5'b0, r_y_min};
			msg_buf_wr = 1'b1;
		end
		
		4'b0011:begin
			msg_buf_in = {5'b0, r_x_max, 5'b0, r_y_max};
			msg_buf_wr = 1'b1;
		end
		...
		4'b0110:begin
			msg_buf_in = {5'b0, m_x_min, 5'b0, m_y_min};
			msg_buf_wr = 1'b1;
		end
		
		4'b0111:begin
			msg_buf_in = {5'b0, m_x_max, 5'b0, m_y_max};
			msg_buf_wr = 1'b1;
		end
		
		4'b1000:begin
			msg_buf_in = {5'b0, t_x_min, 5'b0, t_y_min};
			msg_buf_wr = 1'b1;
		end
		
		4'b1001:begin
			msg_buf_in = {5'b0, t_x_max, 5'b0, t_y_max};
			msg_buf_wr = 1'b1;
		end
		
		4'b1010:begin
			msg_buf_in = {5'b0, l_x_min, 5'b0, l_y_min};
			msg_buf_wr = 1'b1;
		end
		
		4'b1011:begin
			msg_buf_in = {5'b0, l_x_max, 5'b0, l_y_max};
			msg_buf_wr = 1'b1;
		end
		
		4'b1100:begin
			msg_buf_in = {5'b0, n_x_min, 5'b0, n_y_min};
			msg_buf_wr = 1'b1;
		end
		
		4'b1101:begin
			msg_buf_in = {5'b0, n_x_max, 5'b0, n_y_max};
			msg_buf_wr = 1'b1;
		end
		
		4'b1110:begin
			msg_buf_in = {5'b0, b_x_min, 5'b0, b_y_min};
			msg_buf_wr = 1'b1;
		end
		
		4'b1111:begin
			msg_buf_in = {5'b0, b_x_max, 5'b0, b_y_max};
			msg_buf_wr = 1'b1;
		end*/
